...
describe('displayTag', () => {
  it('maps a slug that is not how the thing is written', () => {
    expect(displayTag('c-sharp')).toBe('c#');
    expect(displayTag('console-log')).toBe('console.log');
  });

  it('returns an unmapped tag unchanged', () => {
  ...
  it('leaves slugs alone that people genuinely write that way', () => {
  ...
  it('returns an unknown tag unchanged rather than throwing', () => {
...
describe('the display map', () => {
   ...
   */
  it('has no entry for a tag no post uses', () => {
