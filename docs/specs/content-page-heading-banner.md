...
2. A **compact** (reduced-height) version of the `Hero` banner above the content, showing the
   cover image only — no title or subtitle text.
...
- As the site owner, I get one reusable compact banner variant and one reusable heading-accent
...
- **`/cv`, `/tags`, `/tag/*`, `/posts/[page]`** — unchanged. They keep the current full-height
  `Hero` with their existing text.
...
| Question | Decision |
|---|---|
| What does the compact banner show on `/uses` and `/privacy-policy`? | **Image only, no text.** The page's own `<h1>` stays in the prose below and receives the purple accent. |
| Which pages get the reduced-height banner? | **Only `/uses` and `/privacy-policy`.** All existing `Hero` usages keep their current height. |
...
3. "Reduced size" means **height**. Target ≈ half the current banner: the existing `Hero` is
   `py-20` (160px padding) plus ~130px of text ≈ 290px tall; the compact variant is a fixed
   `10rem` / 160px band. The image stays full-bleed width.
...
```
src/app/components/Hero.tsx            → add `compact` variant (modified)
src/app/components/Hero.scss           → compact height rules (modified)
src/app/components/Hero.test.tsx       → compact variant tests (modified)
src/app/components/ContentPage.scss    → NEW: `.content-page h1` accent rule
src/app/uses/page.tsx                  → add <Hero compact />, wrap main (modified)
src/app/uses/page.test.tsx             → NEW: no test exists for this page today
src/app/privacy-policy/page.tsx        → add <Hero compact />, wrap main (modified)
...
`src/app/page.tsx`, `src/app/posts/[page]/page.tsx`, `src/app/cv/*`, `src/app/tags/page.tsx`,
...
The compact variant follows the existing prop-driven `Hero` pattern rather than a new component,
so all six current call sites keep working with zero changes:
...
|---|---|
| Component (`Hero.test.tsx`) | `compact` renders no heading and no subtitle; carries `hero--compact`; default/tag/title variants still render their text and do **not** carry `hero--compact` (regression guard for the five untouched call sites). |
| Page (`uses/page.test.tsx`, `privacy-policy/page.test.tsx`) | Exactly one `<h1>` with the right name; `main` carries `content-page`; the banner is rendered. Existing privacy-policy assertions (section headings, links, header) must all still pass. |
| Build (`npm run build`) | Static export succeeds — catches SCSS syntax errors and Next.js server/client boundary mistakes that unit tests miss. |
| Manual | Dev-server check of `/uses`, `/privacy-policy` (changed) and `/`, `/post/<any>`, `/cv` (must be visually identical to `main`), in **both** light and dark themes. |
...
Coverage: no threshold is configured in `jest.config.ts`; the goal is that every new branch
(`compact` true/false) is exercised, not a percentage.
...
**Ask first**
- Changing `Hero`'s default (non-compact) height, spacing, or markup in a way that alters the
  homepage, `/cv`, `/tags`, `/tag/*`, or `/posts/[page]`.
...
1. `/uses` renders a banner above its content: an element with classes `hero hero--compact`,
   containing no heading and no subtitle text.
2. `/privacy-policy` renders the same compact banner.
3. The compact banner's rendered height is ≈160px — visibly about half the full `Hero` — and it
   keeps the existing 4px `--color-primary` bottom border.
...
9. `/cv`, `/tags`, `/tag/<tag>`, and `/posts/2` all render the unchanged full-height `Hero`.
10. `git diff --stat main` touches only the files listed under [Project Structure](#project-structure).
11. `npm run lint` reports no new errors or warnings.
12. `npm run test:ci` passes, including every pre-existing assertion in
    `privacy-policy/page.test.tsx` and `Hero.test.tsx`.
13. `npm run build` completes and the static export contains `out/uses.html` and
    `out/privacy-policy.html`, both carrying `hero--compact` and `content-page`.
...
- Adopting `<Hero compact />` on `/post/[slug]`, should the banner-on-article-pages question ever
...
- **Compact height.** `10rem` / 160px is derived from "about half" the current banner. If it reads
  too tall or too short against the cover image's focal point once on screen, it is a one-line
  change in `Hero.scss`. Flag during the manual visual check rather than pre-emptively.
